...
public class AssetSetter {
    ...
    public void setMonster() {
        if (Integer.parseInt(gp.ui.dFormat.format(gp.ui.playTime)) == 15 * timeMultiplicator) {
            ...
            int worldXMaxColHalfScreen = 14;
            int worldYMaxRowHalfScreen = 9;
            timeMultiplicator++;
            for (int i = 0; i < 5 * timeMultiplicator; i++) {
                Random random = new Random();
                int sideRandom = random.nextInt(8) + 1;
                int sideHeightRandomPosition = random.nextInt(9);
                int sideWidthRandomPosition = random.nextInt(14);
                monsterSpawn[i] = new MON_GreenSlime(gp);
                switch (sideRandom) {
                    case 1:
                        monsterSpawn[i].worldX = gp.player.worldX + gp.tileSize * worldXMaxColHalfScreen;
                    ...
                    case 5:
                        ...
                        monsterSpawn[i].worldY = gp.player.worldY - gp.tileSize * worldYMaxRowHalfScreen;
                        break;
                    case 6:
                        monsterSpawn[i].worldX = gp.player.worldX - gp.tileSize * sideWidthRandomPosition;
                        monsterSpawn[i].worldY = gp.player.worldY - gp.tileSize * worldYMaxRowHalfScreen;
